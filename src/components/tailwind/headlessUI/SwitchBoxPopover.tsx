...
export default function SwitchBoxPopover({
  state,
  changeStockPreferenceState,
  openDirection,
  compactStyle,
}: SwitchBoxPopover) {
  const handleClick = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) =>
    handleSwitch(e, state, changeStockPreferenceState);
        ...
        {({ open }) => (
          <>
            ...
            <Transition
              ...
              <Popover.Panel
                className={` ${openDirection === "left" ? "right-0" : openDirection === "right" ? "left-0" : null} absolute  z-10 mt-3 w-screen max-w-md transform px-4 sm:px-0`}>
                        ...
                        <div className="ml-4 flex flex-row">
                          <CustomSwitch
                            ...
                            enabled={
                              state
                                ? state[
                                    item.stateName as keyof StockPresencePreferences
                                  ]
                                : false
                            }
                          />
