...
const PageNumbers = ({ paginatorState, setPaginatorState }: PageNumbers) => {
  ...
  const itemsArray = Array.from({ length: paginatorState.totalPages }, (_, index) => index);

  return (
    <>
      {itemsArray.map((item, index) => (
        <a
          className={`${paginatorState.activePage === index + 1 ? "active" : ""
            }  paginator-number`}
          key={index}
          onClick={() => setPaginatorState({ ...paginatorState, activePage: index + 1 })}
        >
          {item + 1}
        </a>
      ))}
